...
public class JinchuuR : SkillBase
{
    ...
    private int[] attackCounts = { 6, 6 };
    private float[] damages = { 50f, 80f };
    private float[] cooldowns = { 100f, 85f };
    private float adRatio = 0.4f;
    private float singleBonus = 1.15f;  // 단일 대상 15% 추가
    private float cooldownRefund = 0.30f; // 처치 관여 시 30% 환급

    private float duration = 2f;     // 환영 지속 시간
    ...
    IEnumerator PhantomRoutine()
    {
        ...
        if (gotKill)
        {
            ...
            lastUsedTime -= refund;
            Debug.Log($"처치 관여! 쿨타임 {refund}초 환급");
        }
