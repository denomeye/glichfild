...
public class KakaruR : SkillBase
{
    ...
    private float[] cooldowns = { 90f, 75f, 60f };
    private float manaCostFixed = 110f;

    // R 효과 수치
    private float duration = 10f;   // 지속시간
    private float armorPenBonus = 0.20f; // 방어 관통 보정 20%
    ...
    IEnumerator MoveSpeedRoutine()
    {
        ...
        // 이속 복구 (단, 아직 R이 활성화 중이므로 원래값으로만 복구)
        if (isActive)
